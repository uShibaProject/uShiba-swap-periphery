...
import '@uShibaProject/uShiba-swap-lib/contracts/math/SafeMath.sol';
import '@uShibaProject/uShiba-swap-lib/contracts/token/BEP20/IBEP20.sol';
import '@uShibaProject/uShiba-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
import '@uShibaProject/uShiba-swap-lib/contracts/access/Ownable.sol';

import './uShibaToken.sol';

contract uShibaMaster is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of uShibas
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (userInfo.amount * pool.accuShibaPerShare) - userInfo.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accuShibaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // How many allocation points assigned to this pool. uShibas to distribute per block.
        uint256 lastRewardBlock; // Last block number that uShibas distribution occurs.
        uint256 accuShibaPerShare; // Accumulated uShibas per share, times 1e12. See below.
        bool exists; //
    }
    // uShiba tokens created first block.
    uint256 public uShibaStartBlock;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when uShiba mining starts.
    uint256 public startBlock;
    // Block number when bonus uShiba period ends.
    ...
    // uShiba tokens created at bonus end block.
    uint256 public uShibaBonusEndBlock;
    // max reward block
    ...
    // Accumulated uShibas per share, times 1e12.
    uint256 public accuShibaPerShareMultiple = 1E12;
    // The uShiba TOKEN!
    uShibaToken public uShiba;
    // Dev address.
    ...
    constructor(
        ...
        uShiba = _uShiba;
        devAddr = _devAddr;
        uShibaStartBlock = _uShibaStartBlock;
        ...
        // uShiba created when bonus end first block.
        // (uShibaStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        uShibaBonusEndBlock = uShibaStartBlock
        ...
        // max mint block number, _uShibaInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_uShibaInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(uShibaBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function add(
        ...
        pool.accuShibaPerShare = 0;
        pool.exists = true;
        poolAddresses.push(_pair);
    }

    // Update the given pool's uShiba allocation point. Can only be called by the owner.
    function set(
        address _pair,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_uShibaInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            totalReward = tempTo
                ...
                .mul(_uShibaInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _uShibaInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempuShibaInitBlock = _uShibaInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(tempuShibaInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference)))
                    );
                }
            }
        }
    ...
    function pendinguShiba(address _pair, address _user) external view returns (uint256) {
        ...
        uint256 accuShibaPerShare = pool.accuShibaPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 uShibaReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            accuShibaPerShare = accuShibaPerShare.add(uShibaReward.mul(accuShibaPerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(accuShibaPerShare).div(accuShibaPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 uShibaReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        uShiba.mintTo(devAddr, uShibaReward.div(100));
        uShiba.mintTo(address(this), uShibaReward);
        pool.accuShibaPerShare = pool.accuShibaPerShare.add(uShibaReward.mul(accuShibaPerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to uShibaMaster for uShiba allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.accuShibaPerShare).div(accuShibaPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
        ...
        userInfo.rewardDebt = userInfo.amount.mul(pool.accuShibaPerShare).div(accuShibaPerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from uShibaMaster.
    ...
    // Safe uShiba transfer function, just in case if rounding error causes pool to not have enough uShibas.
    function safeuShibaTransfer(address _to, uint256 _amount) internal {
        uint256 uShibaBal = uShiba.balanceOf(address(this));
        if (_amount > uShibaBal) {
            uShiba.transfer(_to, uShibaBal);
        } else {
            uShiba.transfer(_to, _amount);
        }
    }
