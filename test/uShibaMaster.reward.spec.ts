...
// 0.11
const uShibaBonusEndBlock: BigNumber = uShibaStartBlock
  .sub(
    bonusEndBlock
      .sub(startBlock)
...
console.log('uShibaBonusEndBlock  ' + uShibaBonusEndBlock)
...
const getTotalRewardInfoInSameCommonDifference = (
  ...
  console.log('_to ' + _to)
  console.log('_uShibaInitBlock ' + _uShibaInitBlock)
  ...
  console.log('last block create uShiba ' + _uShibaInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)))
  console.log('totalReward ' + totalReward)

  if (lastRewardBulkLastBlock.gt(_from) && lastRewardBulkLastBlock.lte(_to)) {
    let diff = lastRewardBulkLastBlock
      .sub(_from)
      .mul(_uShibaInitBlock.sub(fromBulkNumber.gt(0) ? fromBulkNumber.sub(1).mul(_commonDifference) : 0))
  ...
  if (currentPreviousBulkLastBlock.gt(lastRewardBulkLastBlock)) {
    ...
    if (N.gt(1)) {
      ...
      const diff = _bulkBlockSize.mul(_uShibaInitBlock.sub(currentBulkNumber.sub(2).mul(_commonDifference)))
      totalReward = totalReward.add(diff)
      console.log('diff ' + diff)
    }
  }
...
describe('uShibaMaster', () => {
  it('getReward', () => {
    ...
    let uShibaInitBlock = uShibaStartBlock
    let bulkBlockSize = bonusBeforeBulkBlockSize
    let commonDifference = bonusBeforeCommonDifference
    console.log('--------------------------')
    console.log('from ' + from)
    console.log('to ' + to)
    console.log('uShibaInitBlock ' + uShibaInitBlock)
    console.log('bulkBlockSize ' + bulkBlockSize)
    console.log('commonDifference ' + commonDifference)
    let totalReward = getTotalRewardInfoInSameCommonDifference(from, to, uShibaInitBlock, bulkBlockSize, commonDifference)
